...
\citep{Huang2014}. In remote sensing image analysis, these procedures
<!--
% PEDRO: nao gosto do "these procedures", porque o pos-processamento e' uma etapa, e nao um conjunto de procedimentos
-->
combine pixel-based classification methods with a spatial post-processing
<!--
% PEDRO: aqui aparece post-processing novamente como sendo parte do pos-processamento
-->
method to remove outliers and misclassified pixels. For pixel-based classifiers,
post-processing methods enable including spatial information
in the final results.

<!--
% PEDRO: a primeira frase do paragrafo abaixo me parece desnecessaria, pois repete
% um pouco o que esta na ultima
-->
...
the neighborhood. This technique is rather simplistic; more sophisticated methods 
use class probabilities. 
<!--
% PEDRO: este paragrafo esta com ordem meio invertida. A frase acima fala das
% probabilidades, mas elas sao apresentadas apenas na frase abaixo
-->
For each pixel, machine learning and other statistical 
...
\end{equation}
<!--
% PEDRO: esse p_{i,k} > 0 acho que ficaria melhor na frase acima: "let p_{i,k} > 0"
E' p_{i,k} > 0 ou seria p_{i,k} >= 0?
-->

To make the inference tractable, the class probability values $p_{i,k}$ are converted to 
log-odds values. The logit function converts probability values in the interval $[0,1]$ to logit values in  $[-\infty, \infty]$, expressed by
...
cases, the post-classification smoothing method borrows strength from the neighbors.

## Bayesian inference

After transforming probability values into logits, the problem can be expressed 
...
the machine learning classification is $x_{i,k}$,
<!--
% PEDRO: nao entendi acima
-->
 the aim is to estimate the actual 
...
neighborhood of the pixel have the same distribution. Estimating the local means and variances 
for the prior distribution considers a spatial neighborhood $N_{i}$ close to pixel $p_i$. 
Let $\#(N_{i})$ be the number of elements in the neighborhood $N_{i}$. 
...
When the variance of the likelihood $\sigma^2_k$ increases, the method gives more weight to the neighborhood mean $m_{i,k}$.
...
The input for post-classification is an image with probabilities produced by a machine learning algorithm. This file should be multi-band, where each band contains the pixel probabilities of a single class; it should be GDAL-readable with INT2S data type with values ranging  from [0..10000].
<!--
%PEDRO: como sao numeros inteiros, acho melhor explicar que o 10000 representa 100%, e' isto?
-->
The file name must have information on reference dates and include a version number. In the examples, we use a file produced by a random forests algorithm applied to a data cube of Sentinel-2 images for tile "20LLQ" in the period 2020-06-04 to 2021-08-26.
...
The non-smoothed labeled map shows the need for post-processing. This map is obtained by taking the class of higher probability to each pixel, without considering the spatial context. This is done by \code{sits_label_classification()} whose main parameters are: (a) \code{cube}, a probability cube; (b) \code{output_dir}, directory for storing results; (c) \code{version}. 
```{r, echo=TRUE, eval=TRUE}
...
The impact of Bayesian smoothing can be best captured by producing a labeled map using \code{sits_label_classification()}, taking the smoothed image as its input. Figure \@ref(fig:smth1) shows that the outliers and isolated pixels have been removed.
