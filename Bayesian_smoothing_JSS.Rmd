...
The main idea behind our post-processing method is that a pixel-based classification should take into account its neighborhood pixels. Consider Figure \@ref(fig:mapnosmooth) which shows a class assignment produced by a random forest algorithm on a image time series in the area shown in Figure \@ref(fig:roim). The image time series has been classified using the \proglang{R} package \pkg{sits}[@Simoes2021]. 
...
## Empirical Bayes statistics with anisotropic neighbourhoods
...
These area variations can be best understood by considering the spatial distribution of class probabilities shown in Figure \@ref(fig:pcube). Classes \code{ClearCut_Veg} and \code{ClearCut_Burn} are transitional classes that represent intermediate stages of conversion between areas of class \code{Forest} and those of class \code{ClearCut_Soil} which are more stable classes. The number of outliers of the transitional classes is larger than those of the more stable ones. Thus, it is natural that the smoothing procedure decreases the area of the first two classes and increases that area of last two, resulting in a more informative map. 
